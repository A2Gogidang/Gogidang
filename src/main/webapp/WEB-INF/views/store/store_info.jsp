...
<body>
	...
	<table border="1" align=center>
		...
		<tr>
			<td><button
					onclick="location.href='./review_reg.re?s_num=${storeVO.getS_num() }'">리뷰작성</button></td>
		</tr>
		<tr>
...
<%--  		<td>
		...
		<td>
			<table border="1" style="height:300px; width:400px;">
				<tr align="center">
					<td>상품명</td>
					<td>${menu_List.menuName }</td>
				</tr>
				<tr align="center">
					<td>가격</td>
					<td><fmt:formatNumber pattern="###,###,###" value="${menu_List.price}" /></td>
				</tr>
				<tr align="center">
					<td>등급</td>
					<td>${menu_List.gram }</td>
				</tr>
			</table>		
		</td> --%>
		 <c:forEach items="${menuList}" var="menuList">
			<div class="menu">
				...
				<form role="form" method="post">
					<input type="hidden" id="menunum" name="menunum" value="${menuList.menu_num}" />
				</form>
				<div class="menuImg">
				...
				<div class="menuInfo">
					<p class="menuName">
						<span>상품명</span>${menuList.menu_name}</p>

					<p class="menuMeat">
						<span>상품종류</span>${menuList.meat}</p>

					<p class="menuPrice">
						<span>가격 </span>
						<fmt:formatNumber pattern="###,###,###" value="${menuList.price}" />
						원
					</p>
					...
					<p class="menuStock">
						...
						<fmt:formatNumber pattern="###,###,###"
							value="${menuList.menu_Stock}" />
						EA
					</p>

					<c:if test="${menuList.menu_Stock != 0}">

						<p class="cartStock">
							<span>구입 수량</span>
							<button type="button" class="plus">+</button>
							<input type="number" class="numBox" min="1"
								max="10" value="1" readonly="readonly" />
							<button type="button" class="minus">-</button>

							<input type="hidden" value="${menuList.menu_Stock}"
								class="menuStock_hidden" />
											...
											function() {
												...
												qnastorePageFooter
														.on(
																"click",
																"li a",
																function(e) { //페이지 번호를 클릭했을때 새로운 댓글 가져옴
																	e
																			.preventDefault();
